...
export const updateExpenseInputSchema = {
  ...
  expense_id: z
    .number()
    .optional()
    .describe("Stable expense ID (from list_expenses). Takes priority over description match."),
  description: z
    .string()
    .optional()
    .describe("Substring to match the expense description (case-insensitive). Used if expense_id not provided."),
...
Updates an existing expense in a Wanderlog trip. Target by expense_id (preferred, from
list_expenses) or by description substring match.

Modifiable fields: amount, description, category, currency, date, linked place.
At least one modification field must be provided.

Primary use case: move an expense to a different place without delete+recreate.
`.trim();
...
export async function updateExpense(
  ...
  try {
    if (!args.expense_id && !args.description) {
      return {
        content: [{ type: "text", text: "Provide expense_id or description to target an expense." }],
        isError: true,
      };
    }
    ...
    let expense: ExpenseEntry | undefined;
    ...
    // Mode 2: description match
      ...
      if (matches.length > 1) {
        const lines = matches
          .slice(0, 5)
          .map((m, i) => "  " + (i + 1) + '. [' + m.expense.id + '] "' + m.expense.description + '" — ' + m.expense.amount.currencyCode + " " + m.expense.amount.amount)
          .join("\n");
        return {
          content: [{ type: "text", text: '"' + args.description + '" matches ' + matches.length + " expenses:\n" + lines + "\n\nUse expense_id for precision." }],
          isError: true,
        };
      }
      targetIndex = matches[0]!.index;
      expense = matches[0]!.expense;
    }

    if (targetIndex < 0 || !expense) {
      return { content: [{ type: "text", text: "No expense found." }], isError: true };
    }

    const basePath = ["itinerary", "budget", "expenses", targetIndex];
    const ops: Json0Op[] = [];
    ...
    if (args.new_date) {
      ops.push({ p: [...basePath, "date"], od: expense.date, oi: args.new_date });
      if (expense.associatedDate !== undefined) {
        ops.push({ p: [...basePath, "associatedDate"], od: expense.associatedDate, oi: args.new_date });
      }
    ...
    if (args.new_place_ref) {
      ...
      if (newDate && expense.associatedDate !== undefined) {
    ...
    if (ops.length === 0) {
      return {
        content: [{ type: "text", text: "No changes specified (provide at least one new_* field)." }],
        isError: true,
      };
    }
